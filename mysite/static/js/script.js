...
function cleanUpandFilter(username) {
...
function getTemp(city) {
		//Enter key below example  "&appid=2113325345";
...
function refreshTable(x, username) {
    ...
    for (var row_count = 0; row_count != x.length; ++row_count) {
				var user = x[row_count].owner_id.toLowerCase();
        username = username.toLowerCase();
		if((username === user) || (username === "none"))
		{
		
			//TODO: ISSUES here
				//if (user.localeCompare(username)) || user.localCompare("none")) {
        const row = table.insertRow();
